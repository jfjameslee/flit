...
void enableRawMode(void) {
  if (tcgetattr(STDIN_FILENO, &E.old_termios) == -1)
    fail("tcgetattr");
  ...
  raw.c_iflag &= ~(BRKINT | ICRNL | INPCK | ISTRIP |
                   IXON);  // Disable input for control characters.
  raw.c_oflag &= ~(OPOST); // Disable output processing
  raw.c_cflag |= (CS8);    // Character size: 8 bits per byte
  raw.c_lflag &=
      ~(ECHO | ICANON | ISIG |
        IEXTEN); // Disabling echo, canon' input, signals, etc. NOT the bitflags
  raw.c_cc[VMIN] = 0;
  raw.c_cc[VTIME] = 1;

  if (tcsetattr(STDIN_FILENO, TCSAFLUSH, &raw) == -1)
    fail("tcsetattr"); // Apply change after pending output written to terminal
}
...
void editorInsertNewline(void) {
  if (E.cx == 0) {
    ...
    erow *row = &E.row[E.cy];
    editorInsertRow(
        E.cy + 1, &row->chars[E.cx],
        row->size -
            E.cx); // Split the current row in 2. Divide @ cusor position.
    row = &E.row[E.cy];
    row->size = E.cx;
    row->chars[row->size] = '\0';
    editorUpdateRow(row);
...
int editorCollectSelection(void) {
  if (E.selecting) {
    ...
    if (sel_start_y > sel_end_y ||
      ...
      sel_end_x = E.dropped_cursor_x;
      sel_end_y = E.dropped_cursor_y;
    }
...
void editorSelectionCopy(void) {
  if (E.selecting) {
    ...
    if (E.selection_end_y == E.selection_start_y) {
      strncpy(buffer, &E.row[E.selection_end_y].chars[E.selection_start_x],
              buffer_len);
      buffer[buffer_len] = '\0';
    } else {
      strncpy(buffer, &E.row[E.selection_start_y].chars[E.selection_start_x],
              E.row[E.selection_start_y].size - E.selection_start_x);
      index += E.row[E.selection_start_y].size -
               E.selection_start_x; // No. Characters from first line
      ...
      buffer[index] = '\n';
      index++;
      strncpy(&buffer[index], E.row[E.selection_end_y].chars,
              E.selection_end_x);               // Last characters on last line
...
void editorPaste(void) {
  if (E.copy_buffer) {
    ...
    editorSetStatusMessage("Pasted %d characters @ %d,%d", E.copy_buffer_len,
                           E.cx, E.cy);
  } else {
...
void editorSelectionDelete(void) {
...
void editorSelectionIndent(void) {
...
void editorSelectionUnindent(void) {
  editorCollectSelection();
  ...
  for (y = 0; y < E.screenrows; y++) {
    ...
    if (filerow >= E.numrows) {
      ...
      for (j = 0; j < len; j++) {
        // Selection
        if (E.selecting) {
          // editorSetStatusMessage("Selection: %d/%d -> %d/%d",
          // E.selection_start_x, E.selection_start_y, E.selection_end_x,
          // E.selection_end_y);

          // Check if current character is within selection bounds
          if ((filerow > E.selection_start_y ||
               (filerow == E.selection_start_y && j >= E.selection_start_x)) &&
              (filerow < E.selection_end_y ||
               (filerow == E.selection_end_y && j < E.selection_end_x))) {
            abAppend(ab, "\033[43m", 5);
          } else if (filerow == E.selection_end_y && j == E.selection_end_x) {
            abAppend(ab, "\033[0m",
                     4); // Final character in selection resets highlighting.
                         // (Should come after the character)
          }
        }
...
void editorHandleKeyPress(void) {
  int c = editorReadKey();
  ...
  case '\t':
    if (E.selecting) {
      ...
      default:
        editorSetStatusMessage("Invalid selection shift direction: %c",
                               tab_dir);
        break;
      }
    } else {
